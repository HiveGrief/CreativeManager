...
public class ListUtils {
  ...
  public static boolean inList(String search, List<String> list, ListType listType) {
    ...
    for (String s : lowerList) {
      ...
      if (s.startsWith(SEMILICON)
          && s.endsWith(SEMILICON)
          && search.contains(s.substring(1, s.length() - 1))) return !listType.isBlacklistMode();
      if (s.startsWith(SEMILICON) && search.endsWith(s.substring(1)))
