        ...
        if (e.getNewGameMode().equals(GameMode.CREATIVE)) {
          ConfigurationSection cs =
              CreativeManager.getSettings()
                  .getConfiguration()
                  .getConfigurationSection("creative_armor");
          if (cs != null) {
            p.getInventory()
                .setHelmet(
                    ItemsUtils.fromConfiguration(
                        cs.getConfigurationSection("helmet"), e.getPlayer()));
            p.getInventory()
                .setChestplate(
                    ItemsUtils.fromConfiguration(
                        cs.getConfigurationSection("chestplate"), e.getPlayer()));
            p.getInventory()
                .setLeggings(
                    ItemsUtils.fromConfiguration(
                        cs.getConfigurationSection("leggings"), e.getPlayer()));
            p.getInventory()
                .setBoots(
                    ItemsUtils.fromConfiguration(
                        cs.getConfigurationSection("boots"), e.getPlayer()));
          }
        }
      }
