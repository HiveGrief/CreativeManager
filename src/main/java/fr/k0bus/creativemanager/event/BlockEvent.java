...
public class BlockEvent implements Listener {
   ...
   * BlockLog blockLog =
   * plugin.getDataManager().getBlockFrom(block.getLocation());
  ...
  void onBlockGrow(BlockGrowEvent event) {
    ...
    switch (event.getBlock().getType()) {
      ...
      case SUGAR_CANE:
        BlockLog blockLog =
            plugin.getDataManager().getBlockFrom(block.getRelative(BlockFace.DOWN).getLocation());
        if (blockLog != null && blockLog.isCreative()) {
          plugin.getDataManager().addBlock(new BlockLog(block, blockLog.getPlayer()));
        }
        break;
      case PUMPKIN:
      case MELON:
        for (Block b : BlockUtils.getAdjacentBlocks(block)) {
          if (Tag.CROPS.isTagged(b.getType())) {
            ...
            if (bLog != null && bLog.isCreative()) {
              plugin.getDataManager().addBlock(new BlockLog(block, bLog.getPlayer()));
            }
          }
        }
        break;
      case BAMBOO:
        BlockLog bambooLog =
            plugin.getDataManager().getBlockFrom(block.getRelative(BlockFace.DOWN).getLocation());
        if (bambooLog != null && bambooLog.isCreative()) {
          plugin.getDataManager().addBlock(new BlockLog(block, bambooLog.getPlayer()));
        }
        break;
      default:
        break;
  ...
  void onStructureGrow(StructureGrowEvent event) {
    ...
    if (blockLog != null && blockLog.isCreative()) {
      for (BlockState block : event.getBlocks()) {
        plugin.getDataManager().addBlock(new BlockLog(block.getBlock(), blockLog.getPlayer()));
      }
    }
  }
  ...
  void onBlockSpread(BlockSpreadEvent event) {
    ...
    if (blockLog != null && blockLog.isCreative()) {
      plugin.getDataManager().addBlock(new BlockLog(event.getBlock(), blockLog.getPlayer()));
    }
  }
}
