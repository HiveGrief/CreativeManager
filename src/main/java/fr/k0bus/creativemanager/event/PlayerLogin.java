      ...
      if (CreativeManager.getSettings()
        ...
        if (!CreativeManager.getUpdateChecker().isUpToDate()) {
          ...
          BaseComponent[] button =
              TextComponent.fromLegacyText("§7§l>> §r§5Click here to go on Spigot Page");

          message.setColor(ChatColor.GOLD);
          version.setColor(ChatColor.GREEN);
          for (BaseComponent buttonComponent : button) {
            buttonComponent.setClickEvent(
                new ClickEvent(
                    ClickEvent.Action.OPEN_URL, "https://www.spigotmc.org/resources/75097"));
          }
          BaseComponent[] component =
              new ComponentBuilder().append(tag).append(message).append(version).create();
          e.getPlayer().spigot().sendMessage(component);
          e.getPlayer().spigot().sendMessage(new ComponentBuilder().append(button).create());
        }
      }
    }
