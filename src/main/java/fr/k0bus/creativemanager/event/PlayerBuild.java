...
public class PlayerBuild implements Listener {
  ...
  public void checkBuild(BlockPlaceEvent e) {
    ...
    if (p.getGameMode() == GameMode.CREATIVE) {
      if (CreativeManager.getSettings().getProtection(Protections.BUILD)
          && !p.hasPermission("creativemanager.bypass.build")) {
        if (CreativeManager.getSettings().getConfiguration().getBoolean("send-player-messages"))
          CMUtils.sendMessage(p, "permission.build");
        e.setCancelled(true);
      }
      if (CreativeManager.getSettings().getProtection(Protections.BUILD_CONTAINER)
          && !p.hasPermission("creativemanager.bypass.build-container")) {
        if (e.getBlock() instanceof Container container) {
          CMUtils.sendMessage(p, "permission.build-nbt");
          container.getInventory().clear();
          container.update();
          if (!container.getInventory().isEmpty()) e.setCancelled(true);
        }
      }
    }
  }
