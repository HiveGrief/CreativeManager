...
public enum Protections {
  ...
  ENTITY(
      "entity", Material.ITEM_FRAME, "Protect entity usage (ItemFrame / ArmorStand ...)", "Entity"),
  ...
  PL_CHESTSHOP(
      "plugins.chestshop", Material.GOLD_NUGGET, "Deny using ChestShop", "Plugins - ChestShop"),
  ENCHANT_AND_POTION(
      "enchant-and-potion",
      Material.SHEARS,
      "Deny player to get Enchanted items / Potions with effects",
      "Enchant / Potion"),
  GUI(
      "gui",
      Material.ENDER_CHEST,
      "Deny player to open any GUI (Including vanilla container)",
      "GUI"),
  PL_CITIZENS(
      "plugins.citizens",
      Material.NAME_TAG,
      "Deny player to interact with Citizens",
      "Plugins - Citizens"),
  CUSTOM_NBT(
      "custom-nbt", Material.NAME_TAG, "Deny player to get items with custom NBT", "Custom NBT"),
  ARMOR("armor", Material.CHAINMAIL_HELMET, "Set a defined player armor set", "Creative Armor"),
  PL_ITEMSADDER(
      "itemsadder",
      Material.REDSTONE_BLOCK,
      "Deny player to use itemsadder features",
      "ItemsAdder"),
  BUILD_CONTAINER(
      "build-container",
      Material.CHEST,
      "Deny player to place block with items in inventory",
      "Place container"),
  REMOVE_EFFECTS(
      "remove-effects",
      Material.POTION,
      "Remove all potion effect when player switch gamemode",
      "Remove effects");
  ...
  public ItemStack getIconItem(boolean value) {
    ...
    if (itemMeta != null) {
      ...
      itemMeta.setDisplayName(
          ChatColor.RESET + String.valueOf(ChatColor.BOLD) + ChatColor.GOLD + this.displayName);
      if (value) itemMeta.addEnchant(Registry.ENCHANTMENT.iterator().next(), 1, true);
